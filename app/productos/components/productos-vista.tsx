...
export default function ProductosVista({ categoria }: ProductosVistaProps) {
  ...
  const [productosSeleccion, setproductosSeleccion] = useState<Producto | null>(
    null
  );
  ...
  const fetchProductos = async (
    ...
    try {
      ...
      const filteredProducts = data.items.filter(
        (product) =>
          product.productos_categorias &&
          product.productos_categorias[0]?.categorias !== null
      );
                ...
                >
                  <Card>
                    <CardContent className="p-4">
                      <Image
                        src={
                          product.productos_imagenes[0]?.url ||
                          "/default-image.jpg"

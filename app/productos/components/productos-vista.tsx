...
export default function ProductosVista() {
  ...
  const handlePageChange = (page: number) => {
    setCurrentPage(page);
    fetchProductos(page, itemsPerPage);
  }

  const handleItemsPerPageChange = (value: number) => {
    setItemsPerPage(value);
    setCurrentPage(1); // Reiniciar la página a 1 cuando se cambia la cantidad de elementos por página
    fetchProductos(1, value);
  }

  const fetchProductos = async (page: number, limit: number) => {
    try {
      const data: ProductosResponse = await getProductos(page, limit);
        ...
        <section className="py-20 bg-background">
          <div className="container mx-auto px-4">
            ...
            <div className="mb-4 flex justify-end items-center">
              ...
              <p className="ml-4 text-gray-600">Página {currentPage} de {totalPages}</p>
            </div>
